...
namespace Project2.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        // my code
        // Manage  Settings
        public async Task<ActionResult> ManageSettings(string id)
        {
            if (id == null)
            {
            ...
            ManageSettingsViewModel model = new ManageSettingsViewModel
            {
                Id = user.Id,
        ...
        // Manage Settings Post
        // POST
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ManageSettings(ManageSettingsViewModel model)
        {
            ...
            //return View(model);
            return RedirectToAction("ManageSettings", new { id = model.Id });
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Avatar(HttpPostedFileBase file)
        {
            if (file != null && file.ContentLength > 0)
            {
                if (Path.GetExtension(file.FileName).ToLower() == ".jpg" || Path.GetExtension(file.FileName).ToLower() == ".png" || Path.GetExtension(file.FileName).ToLower() == ".gif" || Path.GetExtension(file.FileName).ToLower() == ".jpeg")
                {
                    ...
                    filename = User.Identity.Name + extension;
                    var path = Path.Combine(Server.MapPath("~/Content/Images/Avatars"), filename);
                    file.SaveAs(path);

                    // save url to database
                    var userId = User.Identity.GetUserId();
                    var avatarUrl = Url.Content("~/Content/Images/Avatars/" + filename);
                    await userRepo.AddAvatar(userId, avatarUrl);
                }
            }
